...
class NumUtil {
  ...
  /// The parameter [fractionDigits] must be an integer satisfying: `0 <= fractionDigits <= 20`.
  static num? getNumByValueNum(num? value, int fractionDigits) {
    if (value == null) return null;
    String valueStr = value.toStringAsFixed(fractionDigits);
    return fractionDigits == 0
        ? int.tryParse(valueStr)
  ...
  /// 除 (精确相除,防止精度丢失).
  /// divide (without loosing precision).
  static double divideDec(num a, num b) {
  ...
  /// 除
  static double divideDecStr(String a, String b) {
    return (Decimal.parse(a) / Decimal.parse(b)).toDouble();
  }
